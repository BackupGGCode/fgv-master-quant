  ...
  while (true){
    ...
    try{
    	...
    	if(!waitQuoteTimeOut){
    		...
    		SimpleOrder orderASK = this->strategy.tradeASK();
    		this->sendOrder(orderASK);
    		SimpleOrder orderBID = this->strategy.tradeBID();
    		this->sendOrder(orderBID);

			sleep(strategy.cycleTime);

    		this->cancelOrder(orderASK);
    		this->cancelOrder(orderBID);
    		this->resetFlags();
