...
function stopAll($_numAgent, $_agentList){
	...
	while ($i < $_numAgent) {
		...
		if($type == "bband"){
			$agent=mysql_result($_agentList,$i,"id_agent");
			$statusTraders = exec("./scripts/bbandtraders.sh status $agent");
			if($statusTraders == "STARTED"){exec("./scripts/bbandtraders.sh stop $agent > /dev/null &");}
		}
...
function startAll($_numAgent, $_agentList){
	...
	while ($i < $_numAgent) {
		...
		if($type == "bband"){
			$agent=mysql_result($_agentList,$i,"id_agent");
			$statusTraders = exec("./scripts/bbandtraders.sh status $agent");
			if($statusTraders == "STOPPED"){exec("./scripts/bbandtraders.sh start $agent > /dev/null &");}
		}
...
$filename_agents="./results/test_BB_agents.csv";
exportMysqlToCsv($query_agents, $filename_agents);

$ii=1;
while ($ii <= 100) {
	echo "Iniciando teste: $ii \n";
	...
	$filename_prices="./results/test_BB_".$ii."_prices.csv";
	exportMysqlToCsv($query_prices, $filename_prices);
	
	$query_portfolio="SELECT * FROM quickfix.portfolio";
	$filename_portfolio="./results/test_BB_".$ii."_portfolios.csv";
